...
public class CreativeTabRegistry {
    ...
    public static final DeferredHolder<CreativeModeTab, CreativeModeTab> POSTALIS_TAB = CREATIVE_TAB.register("postalis",
            () -> CreativeModeTab.builder().title(Component.translatable("creative.postalis.tab")).icon(() -> ItemRegistry.WIND_BREAKER.get().getDefaultInstance())
                    .displayItems((parameters, output) -> BuiltInRegistries.ITEM.stream().filter(i -> i instanceof IPostalis
                                    || i instanceof BlockItem blockItem && blockItem.getBlock() instanceof IPostalis).map(ItemStack::new).forEach(output::accept)).build());
}
